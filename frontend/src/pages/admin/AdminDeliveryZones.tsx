      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold font-heading text-[#25201D] flex items-center gap-2">
            <MapPin size={24} className="text-[#B85C38]" />
        ...
        {zones.map((zone) => (
          <div
            ...
            className={`p-6 rounded-2xl bg-[#FFFFFF] border transition-all ${
              zone.active ? 'border-[#E8DED6] shadow-lg' : 'border-[#E8DED6] opacity-60'
            }`}
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold font-heading text-[#25201D] text-base">{zone.name}</h3>
                  <span className="text-[10px] text-[#6F6761]">Est: {zone.estimatedMinutes}</span>
                </div>
              </div>
            ...
            <div className="pt-3 border-t border-[#E8DED6] grid grid-cols-2 gap-2 text-xs">
              <div>
                ...
                <span className="text-base font-extrabold text-[#B85C38]">
                  Rs. {zone.fee}
                </span>
              </div>
              <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="max-w-md w-full rounded-3xl bg-[#FFFFFF] border border-[#E8DED6] p-6 sm:p-8 space-y-5 shadow-2xl">
            ...
            <form onSubmit={handleCreateZone} className="space-y-4 text-xs">
              <div>
                <label className="font-bold uppercase text-[#6F6761] block mb-1">Sector Name *</label>
                <input
                  type="text"
                  value={name}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="font-bold uppercase text-[#6F6761] block mb-1">Fee (PKR) *</label>
                  <input
                    type="number"
                    value={fee}
              ...
              <div>
                <label className="font-bold uppercase text-[#6F6761] block mb-1">Est. Minutes</label>
                <input
                  type="text"
