...
export const WhyThreeFlames: React.FC = () => {
  const features = [
    {
      icon: <Flame className="w-8 h-8 text-[#C97845]" />,
    ...
    {
      icon: <Bike className="w-8 h-8 text-[#D6A15D]" />,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          {features.map((feat, index) => (
            <div
              key={index}
              className="p-8 rounded-2xl bg-[#332B25] border border-[#51463D] hover:border-[#D6A15D]/50 transition-all duration-300 group hover:-translate-y-1 hover:shadow-xl hover:shadow-black/30 flex flex-col justify-between"
            >
              <div>
                <div className="w-14 h-14 rounded-2xl bg-[#28221D] border border-[#51463D] flex items-center justify-center mb-6 group-hover:scale-110 group-hover:border-[#D6A15D]/60 transition-all duration-300 shadow-inner">
                  {feat.icon}
                </div>
                <h3 className="text-xl font-bold font-heading text-[#F3EDE5] mb-3">
                  {feat.title}
                </h3>
                <p className="text-sm text-[#BDB1A5] leading-relaxed">
                  {feat.description}
                </p>
              </div>

              <div className="mt-6 pt-4 border-t border-[#51463D]/50 flex items-center gap-2 text-xs font-semibold text-[#D6A15D]">
