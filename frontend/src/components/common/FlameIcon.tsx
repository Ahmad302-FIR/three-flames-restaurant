...
export const FlameIcon: React.FC<FlameIconProps> = ({ className = 'w-6 h-6', size, glow = true }) => {
  return (
    <span className={`inline-flex items-center justify-center relative ${className}`} style={size ? { width: size, height: size } : undefined}>
      {glow && (
        <span className="absolute inset-0 bg-[#F97316]/30 blur-md rounded-full -z-10 animate-pulse" />
      )}
      <svg
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className="w-full h-full text-[#F97316] drop-shadow-[0_0_10px_rgba(249,115,22,0.5)]"
      >
        <path
